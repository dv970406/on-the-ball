...
→ **카카오를 쓰려면 비즈 앱 전환이 사실상 필수입니다.** 개인 개발자도 신청할 수 있습니다.
   ...
   (`src/shared/config/oauth.ts`의 `OAUTH_PROVIDERS`에서 `"kakao"`를 잠시 빼면 버튼이 사라집니다).
...
⚠ **이메일은 필수 동의로 둡니다.** 프로필 화면이 "로그인 계정 · {이메일}"로 표시하고
(`views/profile/ui/profile-view.tsx`), 이메일이 같고 검증되면 Supabase가 identity를 자동으로
합쳐 계정이 갈리는 것을 줄여줍니다.

⚠ 닉네임은 **이메일에서 뽑지 않습니다** — 가입 시 `random_nickname()`이 배정하고 사용자가
프로필에서 바꿉니다. 그래서 `email_optional = true`를 켜 둔 채여도 이메일이 없는 사용자의
로그인 자체는 성공합니다(표시만 비어 있음).
