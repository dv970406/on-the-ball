...
(⚠ 예외가 하나 있다 — `app/admin-you-can-not-access/layout.tsx`의 `rpc("is_admin")`은
화면을 그리려는 조회가 아니라 **가드**라 이 표의 대상이 아니다. 사유는 아래 어드민 절에.)
...
| `app/matches/ranking/page.tsx` | 랭킹 SSR(시즌 전체 + 최근 라운드) | 지금은 불필요(메타데이터가 정적). ⚠ 세션이 없으면 **익명 클라이언트**다(`hasSessionCookie()` 갈림) — 비로그인에게는 `is_me`가 전부 false라 응답이 모든 요청에 같다. ⚠ 범위 조회의 기준 시각은 **분 단위로 내린다** — ms 시각이 URL에 실리면 그 조회만 요청마다 캐시 미스가 되고 엔트리가 요청 수만큼 쌓인다(아래 "익명 조회는 캐시한다"). RPC를 GET(`get: true`)으로 부르는 것은 인자가 URL에 드러나 키를 로그에서 대조할 수 있어서다 — POST도 명시적 `revalidate`가 있으면 캐시된다(실측) |
...
- ⚠ **요청 시각을 조회 조건에 그대로 싣지 않는다.** Data Cache의 키는 URL·메서드·본문이라
  `lte("kickoff_at", new Date().toISOString())`처럼 ms 시각이 들어가면 그 조회는 **매번 미스**이고
  엔트리만 요청 수만큼 쌓인다 — 빌드도 화면도 멀쩡해 드러나지 않는다(실측: 5회 요청에 5건 DB 도달).
  시각이 조건에 필요하면 판정이 달라지지 않는 단위로 **내려서** 싣는다(`fetchMatchRanking`이 분 단위).
...
## 어드민 백오피스 — 서버 가드는 **리다이렉트가 아니라 `notFound()`**

`/admin-you-can-not-access`의 `layout.tsx`가 `supabase.rpc("is_admin")` 한 번으로 판정하고,
관리자가 아니면 `notFound()`한다.

- **`getUser()`가 아니라 RPC다.** `is_admin()`이 `auth.uid()`를 직접 보므로 GoTrue 왕복 없이
  PostgREST 한 번으로 끝난다(proxy가 이미 같은 요청에서 세션을 갱신했다).
- **리다이렉트가 아닌 이유**: proxy에서 라우트 가드를 걷어낸 근거가 "판정자가 둘이면 무한
  리다이렉트가 된다"인데, 404는 이동이 아니라 루프가 생기지 않는다. 덤으로 비관리자에게
  **이 경로의 존재 자체가 드러나지 않는다.**
- ⚠ 조회 실패(`unknown`)도 404로 보낸다. 글 상세는 "일시 장애로 멀쩡한 글을 404로 단정하지
  않는다"가 맞지만 **여기서는 열어 두는 쪽이 위험하다.**
- ⚠ `cookies()`를 try/catch로 감쌌으므로 **`unstable_rethrow`가 필수**다 — 삼키면 이 라우트가
  조용히 정적 프리렌더된다(빌드 로그의 `ƒ`/`○`로만 드러난다).
- **`robots.txt`에는 적지 않는다.** 경로를 그 파일에 쓰는 순간 파일이 경로를 공개한다 —
  화면의 `robots: { index: false }`가 유일하고 충분한 색인 차단이다(크롤러는 404만 본다).
- ⚠⚠ **`title`을 이 layout에 두지 않는다 — 각 page가 갖는다.** layout이 `notFound()`를
  던지면 page의 metadata는 해석되지 않고 **layout의 것만 남아** 404의 제목이 다른 경로의
  404와 달라진다(실측: 임의 경로는 `페이지를 찾을 수 없어요 | 온더볼`인데 여기는 접미사가
  없는 `페이지를 찾을 수 없어요`였다 — 그 차이가 곧 "이 경로에 layout이 있다"는 신호이고,
  `notFound()`를 고른 이유 중 하나였던 "경로의 존재가 드러나지 않는다"를 스스로 깬다).
  `robots`는 남겨도 된다 — 값이 화면에 드러나지 않는다.
- ⚠ **그래도 브라우저 탭 제목까지는 감출 수 없다.** 클라이언트가 하이드레이션에서 page의
  metadata를 적용하므로, 비관리자가 이 경로를 **직접 열면** 화면은 404인데 탭에는
  `공지사항 관리`가 뜬다(프로덕션 빌드에서도 재현). title을 아예 빼도 root 기본값
  `온더볼`이 남아 일반 404와 여전히 갈린다 — 즉 제목으로는 끝까지 막을 수 없다.
  **막히는 것은 서버 응답이고**(HTTP 404 + 임의 경로와 글자까지 같은 HTML) 크롤러·스캐너·
  비-JS 클라이언트에는 구분이 없다. 남는 노출은 **경로를 이미 아는 사람**에게만 닿으므로
...
⚠ **공지는 목록도 상세도 그 창에 걸린다.** 셋 다 익명 클라이언트를 타기 때문이다 —
공지를 등록·수정·삭제하거나 예약 공지의 시작 시각이 지나도 **최대 30초** 늦게 반영된다
(실측: 노출 기간을 연 뒤 목록에 나타나기까지 30초대). 어드민 화면은 자기 RPC를 쓰므로
즉시 정확하고, 어긋나는 것은 서비스 쪽 화면뿐이다. 공지는 초 단위 긴급성이 없어
수용하지만, 그 성질이 필요해지면 갈림을 없애고 쿠키 클라이언트로 되돌린다.

⚠ **무효화는 조치한 관리자 브라우저의 캐시만 갱신한다.** `refetchOnWindowFocus`가 꺼져 있고
...
- ⚠ `runtime = "nodejs"`가 필수다(Edge에는 `node:fs`가 없어 `scripts/lib/*.mjs` import가 깨진다).
- ⚠ **`maxDuration`을 명시하고 행 단위 폴백에 상한을 준다.** 없으면 일괄 실패 시 380행을
  하나씩 왕복해 타임아웃이 "부분 적용 + 응답 없음"이 된다.
