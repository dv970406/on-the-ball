...
⚠ `name`(정식)과 `short_name`(약칭)의 **쓰임이 다르다.** 카드·상세 제목은 정식명이고,
예측 버튼 라벨은 약칭이다 — 세로로 쌓인 버튼에서 "맨체스터 유나이티드 승"은 좁은 화면에서
잘리는데, **잘린 팀 이름은 어느 팀인지 알 수 없어 고를 수가 없다.**
