...
세션의 유효성을 두 곳이 **다른 방법으로** 판정한다 — 서버는 `getUser()`로 GoTrue에 물어보고
(proxy가 갱신할 때), 클라이언트는 쿠키의 `expires_at`만 로컬 검사한다. 다른 기기의 global
로그아웃·계정 삭제·JWT 시크릿 회전이 일어나면 **토큰은 만료 전인데 서버는 403을 준다.**

그러면 화면은 로그인 상태를 그리는데 **모든 조회가 RLS에 막혀 실패한다** — 사용자는 자기가
로그인돼 있다고 믿는 채로 아무것도 못 한다. 스스로 빠져나올 방법이 없는 상태다.

⚠ **이 불일치에 리다이렉트를 얹지 않는다.** 한쪽이 "비로그인"이라 내보내고 다른 쪽이
"로그인"이라 되돌리면 **무한 루프**가 되어 탈출 수단이 사라진다. proxy에서 라우트 가드를
걷어낸 이유 중 하나이고, 새 가드를 만들 때도 같다 — **판정자를 늘리지 말고 아래처럼 정리한다.**
...
- **시각에 의존하는 표시는 전부 `nowMs`를 인자로 받는다** — `formatRelativeTime(iso, nowMs)`·`isHotPost(post, nowMs)`. `null`(서버·첫 렌더)이면 상대시각 대신 절대시각을, HOT 배지는 아무것도 그리지 않는다. **목록·상세가 모두 SSR이라 이건 선택이 아니다.**
- ⚠ 지배 변수는 SSR↔하이드레이션 지연이 아니라 **사용자 기기의 시계 오차**다(서버는 NTP 동기 시각, 브라우저는 기기 시각). 그래서 "서버가 렌더한 값을 클라가 다시 계산"하는 형태를 만들면 안 된다 — 기준 시각을 **인자로 흘려보내야** 한다.
- ⚠ **SSR 화면은 서버 시각을 함께 내려보낸다**(`serverNowMs ?? useNowMs()` — 순서는 아래). 안 그러면 첫
  렌더가 절대시각이 되었다가 마운트 직후 상대시각으로 바뀌어 **글자 폭이 달라지는 시프트**가
  된다 — 하이드레이션은 맞는데 화면이 흔들리는, FOUC처럼 보이는 상태다.
- ⚠ **순서는 `serverNowMs ?? useNowMs()`다 — 반대로 두지 않는다.** `useNowMs()`는 마운트당이
  아니라 **모듈 스코프에 세션당 한 번** 고정되므로(앱을 처음 연 화면에서 굳는다), 클라 값을
  앞에 두면 **갓 받은 서버 시각을 낡은 클라 시계가 이긴다.** 동적 라우트는 이동할 때마다
  서버가 새로 찍어 주므로(Router Cache TTL 0) 서버 값이 언제나 더 신선하다.
  - 실측: `/posts`를 연 뒤 43초 기다렸다가 탭으로 이동하니 **41초 전에 킥오프한 경기가
    "다가오는 경기"에 그려졌고**, 상세에서는 예측 버튼이 활성인 채 마감 후 분포가 함께 떴다.
  - ⚠ **표시용·기능용을 가르지 않는다.** HOT 배지처럼 장식인 자리도 같은 순서를 쓴다 —
    "여기는 표시니까 아무거나"라는 경계는 반드시 갈리고, 실제로 `isSurveyOpen`·`isMatchOpen`은
    같은 값으로 **참여 가능 여부**를 가른다(장식이 아니다).
  - ⚠ `??`는 단축평가라 **훅을 뒤에 두면 조건부 호출**이 된다 → `const clientNowMs = useNowMs();`로
    먼저 무조건 부르고 값만 고른다.
...
| 가드를 둔다 | 두지 않는다 |
|---|---|
| **행이 생긴다** — 글 작성(`usePostSubmit`), 댓글 작성(`useCommentComposer`) | **소셜 로그인** — 버튼을 누르면 페이지가 프로바이더로 넘어가 화면 자체가 사라진다 |
...
대개는 훅이다 — features일 수도(`useUnlinkIdentity`) 화면 슬라이스의 `model/`일 수도 있고(per-call 콜백이 이동 목적지·입력창 복원 같은 **상위 레이어의 결정**을 담고 있으면 features로 내리지 않는다 — `usePostSubmit`·`usePostUpdate`가 그 자리다),
`mutate`를 인라인으로 부르는 컴포넌트라면 그 컴포넌트다(`ReportReasonList` — 시트 안 항목 하나가 곧 제출이라 조립할 훅이 따로 없다).
**기준은 "훅인가"가 아니라 "그 뮤테이션의 `status`를 직접 읽는가"** 다 — 아래 예외 항목이 그 이유다.

⚠ **예외를 두지 않는다.** 한때 `PostForm`이 자기 제출을 방어했는데(`onSubmit` prop만 받아 그것이 뮤테이션인지 모르므로), **`isPending`을 prop으로 받는 컴포넌트는 가드를 들 수 없다** — 부모가 리렌더되기 전까지 자식은 낡은 값을 다시 읽을 뿐이라 해제 신호가 오지 않고, 첫 실패 이후 그 화면에서 영영 제출할 수 없게 된다(실측: 등록 3연타 → 요청 1건). 폼은 검증까지만 하고 유효한 입력만 올려보내며, 가드는 뮤테이션을 조립하는 화면 슬라이스의 `model/` 훅(`usePostSubmit`·`usePostUpdate`)이 갖는다.
...
⚠ 검증은 **`safeNextPath` 하나가 소유한다** — 직접 문자열 검사를 짜지 않는다.
