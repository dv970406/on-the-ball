/**
 * 본문 이미지의 목표 용량. **이 값을 넘기지 않는 것이 계약이다.**
 *
...
/**
 * 이미지를 **비율을 유지한 채** webp로 줄인다. 결과는 항상 `TARGET_BYTES` 이하다.
 *
 * ⚠ `features/update-profile`의 `resizeToAvatar`와 뼈대가 겹치지만 **일부러 중복이다.**
 *   저쪽은 아바타가 원형이라 가운데를 정사각으로 **잘라내는데**, 본문 사진을 그렇게 자르면
 *   내용이 날아간다 — 형태가 진짜 같지 않으므로 공용화 대상이 아니다("2회뿐이거나 데이터가
 *   서로 다르면 중복을 둔다", code-quality.md). features끼리는 import도 할 수 없어
 *   묶으려면 `shared/lib` 승격이 필요한데, 세 번째 이미지 기능이 생기면 그때 한다.
 *
 * ⚠ **클라이언트 압축은 UX이지 방어가 아니다.** 우회하면 원본이 올라가므로 버킷의
 *   `file_size_limit`·`allowed_mime_types`가 실제 방어선이다(RLS와 같은 구조).
 */
export async function resizeToPostImage(file: File): Promise<Blob> {
  /*
   ...
   */
  if (file.size <= TARGET_BYTES && (await looksLikeWebp(file))) return file;
  ...
  try {
    ...
    console.error("[post] 이미지 디코딩 실패:", e);
  ...
  try {
    ...
    console.error("[post] 목표 용량에 닿지 못함:", smallest?.size);
