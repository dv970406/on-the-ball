 ...
 *
 * ⚠ **제공자 CDN을 직접 걸지 않고 우리가 줄인 사본을 서빙한다.** football-data의 원본은
 *   200×200 PNG(팀당 4~16KB, 20팀 244KB)인데 여기서 그리는 크기는 24·44px이라 8배 가까이
 *   과하다. 리사이즈 파라미터를 지원하지 않아(`?width=` 등이 전부 원본을 준다 — 실측)
 *   줄이려면 사본을 두는 수밖에 없다. 128px 팔레트 PNG로 **20팀 244KB → 약 97KB(60% 감소)**.
