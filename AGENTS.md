...
- **개수를 적지 않는다.** "예외 4곳"·"전량 8개"는 행을 더하는 순간 거짓이 된다 → "아래 표가 전부다"로 쓴다. 개수 **자체가 규칙의 내용**인 경우만 예외다(`api-and-db.md`의 anon EXECUTE 화이트리스트가 그 자리다 — "그중 definer는 몇"이 화이트리스트의 뜻을 이루므로 개수를 빼면 문장이 성립하지 않는다). ⚠ 그런 자리는 **값이 바뀌면 인용하는 쪽까지 함께 고친다** — 예외를 쓰는 순간 두 문서가 한 숫자로 묶인다.
