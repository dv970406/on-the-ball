...
export const config = {
  ...
  // ⚠ **`/api` 아래 핸들러는 자기 세션 검증을 스스로 해야 한다** — 쿠키는 그대로 오지만
  //   여기서 갱신하지 않으므로 필요하면 핸들러가 직접 저장한다(Route Handler는 쿠키를
  //   쓸 수 있다). 어드민 동기화는 `getUser()` → `rpc("is_admin")`으로, 크론 동기화는
  //   `CRON_SECRET`으로 각자 인가한다.
