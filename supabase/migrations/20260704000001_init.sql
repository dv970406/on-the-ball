...
--  * 쓰기 정책(중복 방지·24h 변경 제한·스트릭)은 SECURITY DEFINER RPC로
...
-- 투표: 중복 방지 + 마감 검증 + (유니폼) 재탭 취소 + 24시간 내 1회 변경
...
-- 퀴즈 시도: 중복 방지 + 오픈일 검증 + 정답 판정 + 스트릭 증감
...
-- votes / quiz_attempts 의 쓰기는 정책 없음 → RPC(SECURITY DEFINER)로만 가능
...
-- =====================================================================
-- profiles 컬럼 권한 — 스트릭 위조 및 개인 필드 노출 차단
