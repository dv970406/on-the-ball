...
--
-- ⚠ 예정·만료는 `notice_select_live` 정책이 감춘다 → 일반 조회로는 2건만
--   보이고 admin_notice_list로는 다섯이 다 보이는 것이 정상이다.
-- ⚠ **노출 중인 것이 타입별로 하나씩 있어야 한다.** 필독만 있으면 목록의
--   정렬(필독 먼저)도, 배지가 둘로 갈리는 것도, 피드 배너가 '공지'를 올리지
--   않는다는 것도 화면에서 확인할 수 없다 — 셋 다 조용히 틀려질 수 있는 자리다.
...
insert into public.notice (type, title, body, opens_at, closes_at) values
  ...
  ('공지',
   '승부예측 적중률 집계 기준 안내',
   E'승부예측 적중률은 **채점이 끝난 경기**만으로 계산해요.\n\n' ||
   E'- 취소·몰수된 경기는 집계에서 빠집니다\n' ||
   E'- 스코어가 정정되면 적중 여부도 함께 다시 계산돼요\n\n' ||
   E'프로필 화면에서 언제든 확인할 수 있어요.',
